  ...
  context('With one token added to the staking pools', () => {
    ...
    it('revert if reward limit is zero', async () => {
      ...
      await expectRevert(
        ...
        'constructor: _rewardLimit must be greater than zero'
      );
    });
  });
  ...
  describe('owner functions', () => {
    ...
    it('can not "add" if token is zero address', async () => {
      await expectRevert(
        ...
        'add: _lpToken must not be zero address'
      );
    });

    it('can not "add" if token cap is zero', async () => {
      await expectRevert(
        this.staking.add('100', this.xtp.address, toBn('0'), true, {from: adminAlice}),
        'add: _tokenCap must be greater than zero'
      );
    });
    ...
    it('can not "set" if token cap is zero', async () => {
      await expectRevert(
        ...
        'set: _tokenCap must be greater than zero'
      );
    });
